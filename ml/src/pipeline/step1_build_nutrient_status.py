...
def create_initial_nutrition_class(df: pd.DataFrame) -> pd.Series:
    ...
    # Rough reference kcal used only for class bootstrapping.
    target_kcal = np.where(sex == 1, 2300, 1800).astype(float)
    target_kcal = np.where(age >= 50, target_kcal - 100, target_kcal)
